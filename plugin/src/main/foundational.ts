      ...
      for (const mode of collection.modes) {
        ...
        if (
          ...
          valuesByMode[mode.modeId] = {
            type: "VARIABLE_ALIAS",
            id: alias.id,
            // Prefer stable library key so cross-file catalog resolution works
            // (Variable.id is file-local; key is shared after publish).
            key: aliasedVar?.key || "",
...
function aliasSemantic(raw: unknown): FoundationSemanticValue | null {
  if (
    ...
    const a = raw as { id: string; name: string; key?: string }
    return {
      kind: "alias",
      aliasId: a.id,
      aliasName: a.name,
      ...(a.key ? { aliasKey: a.key } : {}),
    }
  }
  return null
}
 ...
 * Cross-file library aliases use different Variable.ids but share Variable.key.
 */
export function findCatalogToken(
  ...
  if (aliasName) {
    ...
    // Also allow catalog display names with "File / Name" prefix.
